...
export default function TableHeader() {
  ...
  return (
    <div>
      <Card className="mb-[4vh] mt-[2vh] h-[calc(24vh-82px)] overflow-hidden border-[#51689A] bg-[#F6F7FE] shadow-md">
        <CardContent className="flex h-full flex-row items-center justify-between gap-4 overflow-hidden p-0">
          <div className="flex flex-row items-center gap-4 p-8">
            <Users className="text-[#1B2065F2] rounded-md border border-[#1B2065F2]" />
            <p className="font-bold text-[#1B2065F2]">Schedule List</p>
          </div>
          <div className="flex items-center justify-end gap-4 px-5 py-7">
            ...
            <div className="relative flex min-w-[140px] items-center">
              <Funnel
                className="pointer-events-none absolute start-3 text-[#1B2065F2]"
                size={18}
                aria-hidden
              />
              <Select value={audience} onValueChange={setAudience}>
                <SelectTrigger className="h-10 w-full rounded-xl border border-[#1B2065F2] bg-[#FEF9F9] ps-8 text-sm text-[#1B2065F2]">
