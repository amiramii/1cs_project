...
const SideBarItems = ({ 
  ...
  return (
    ...
    {items.map((item) => {
        ...
        return(
            <li key={item.href} className={`${isActive ? 'border-b-2 border-[#51689A] bg-[#51689A]/15 dark:border-[#74A7BD] dark:bg-[#1B2065]/35 md:border-b-0 md:border-s-4 md:border-[#51689A] dark:md:border-[#74A7BD] text-[#1B2065F2] dark:text-[#FEF9F9]' : 'border-b-2 border-transparent text-[#1B2065F2]/85 md:border-s-4 md:border-transparent dark:text-[#FEF9F9]/80'} group relative rounded-t-xl p-3 transition-colors md:w-full md:rounded-none md:rounded-e-xl md:pr-2 md:pe-2 `}>
                <Link
                  href={`${item.href}`}
                  onClick={onItemClick}
                  className={`flex w-full flex-col items-center justify-center gap-2 px-1 py-2 text-center text-[12px] font-semibold leading-tight text-inherit transition-colors focus-visible:bg-[#74A7BD]/25 focus-visible:outline-none focus-visible:ring-0 dark:focus-visible:text-[#FEF9F9] md:px-2 md:text-sm ${
                    expanded
